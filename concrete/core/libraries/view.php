		...
		*/
		protected function setThemeForView($pl, $filename, $wrapTemplateInTheme = false) {
			...
			if ($pl instanceof PageTheme) {
				...
				$theme_path = DIRNAME_THEMES . '/' . $pl->getThemeHandle() . '/';
				$rec = $env->getRecord ($theme_path . $filename, $this->pkgHandle);
				/* WEIYM: hack to try to use view.php in the subdirectory as the theme file */
				if (!$rec->exists()) {
					#error_log ("theme file: $theme_path$filename does not exist\n", 3, "/var/tmp/c5.log");
					$fragments = explode ('/', $filename, -1);
					if (count ($fragments) > 0) {
						$rec = $env->getRecord ($theme_path . $fragments[0] . '/view.php',
							 $this->pkgHandle);
					}
				}
				...
				if (!$rec->exists()) {
					if ($wrapTemplateInTheme) {
						$theme = $env->getPath($theme_path . FILENAME_THEMES_VIEW, $this->pkgHandle);
					} else {
						$theme = $env->getPath($theme_path . FILENAME_THEMES_DEFAULT, $this->pkgHandle);
					}
				} else {
					$theme = $rec->file;
					$this->disableContentInclude = true;
				}
				#error_log ("result theme filename: $theme ($filename)\n", 3, "/var/tmp/c5.log");
		...
		public function render($view, $args = null) { 
			...
			if ($view instanceof Page) {
				...
				if ($view->getCollectionTypeID() == 0 && $cFilename) {
					...
					/* WEIYM: hack to support non-root single pages */
					#$themeFilename = $c->getCollectionHandle() . '.php';
					$themeFilename = ltrim ($viewPath, '/') . '.php';
				} else {
