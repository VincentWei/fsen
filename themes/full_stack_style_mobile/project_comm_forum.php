			...
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h2 class="panel-title">
						...
						<small style="color:#fafafa;"><?php echo h5($prt['part_desc']) ?></small>
					</h2>
				</div>
...
<div class="form-group">
	<button type="button" disabled="disabled" class="btn btn-warning enable-on-logged-in"
		...
		<?php echo t('New Thread') ?>
	</button>
	<p class="help-block hidden-on-logged-in">
		<?php echo t('<a href="/fse_login?redirectURL=%s">Sign in</a> to start a new thread.', $page_path) ?>
	</p>
</div>

<fieldset id="formNewThread" class="collapse">
	<form method="post" enctype="multipart/form-data" class="validate"
			action="/fse_settings/projects/add_new_forum_thread" role="form">
		...
		<div class="form-group">
			<label for="postContent" class="sr-only">
				<?php $my_text = t('New thread content'); echo $my_text ?>
			</label>
	...
	<div class="col-md-8">
		...
		<span class="section-action visible-on-manage-community-right"
			...
			<a class="launch-modal"
					href="<?php echo "/index.php/tools/thread_action.php?fsenDocLang=$doc_lang&cID=$page_id&projectID=$project_id&domainHandle=$domain_handle&volumeHandle=$volume_handle&partHandle=$part_handle&chapterHandle=" . $thd['chapter_handle'] ?>&threadAction=delete">
		...
		<span class="section-action visible-on-manage-community-right"
			...
			<a class="launch-modal"
					href="<?php echo "/index.php/tools/thread_action.php?fsenDocLang=$doc_lang&cID=$page_id&projectID=$project_id&domainHandle=$domain_handle&volumeHandle=$volume_handle&partHandle=$part_handle&chapterHandle=" . $thd['chapter_handle'] ?>&threadAction=untop">
...
<div class="row">
	<div class="col-md-8">
		...
		<span class="section-action visible-on-manage-community-right"
				style="margin-top: 0; margin-right: 5px;">
			<a class="launch-modal"
					href="<?php echo "/index.php/tools/thread_action.php?fsenDocLang=$doc_lang&cID=$page_id&projectID=$project_id&domainHandle=$domain_handle&volumeHandle=$volume_handle&partHandle=$part_handle&chapterHandle=" . $thd['chapter_handle'] ?>&threadAction=top">
