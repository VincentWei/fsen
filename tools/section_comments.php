...
if (!fse_try_to_login ()) {
	...
	if (count ($section_info) == 0) {
		...
		if ($html_content == false || $author_name_info == false) {
			...
			$fetch_earlier_action = $uh->getBlockTypeToolsURL ($bt) . '/fetch_earlier_comments.php';
...
<div class="modal-header">
	...
	<h4 class="modal-title"><?php echo t ('Comments') ?></h4>
</div>
...
<form id="formCommentSection" method="post" action="<?php echo $form_action ?>">
		...
		<div class="col-md-12">
			<div class="input-group">
				<span class="input-group-addon">
					<img class="small-avatar" style="height:20px;"
							src="<?php echo $author_name_info['avatar_url'] ?>" alt="Avatar" />
				</span>
				<input class="form-control" name="commentBody" maxlength="140"
						required="true" placeholder="<?php echo t('Your comment') ?>" />
				<span class="input-group-btn">
					<button id="btnPostComment" class="btn btn-primary" type="submit"
							data-doing-text="<?php echo t('Posting...') ?>"
							data-done-text="<?php echo t('Posted') ?>"><?php echo t('Post') ?></button>
				</span>
			</div><!-- /input-group -->
		</div><!-- /.col-md-12 -->
	</div><!-- row -->
...
?>
				<li id="liComment<?php echo $comment['id'] ?>" class="list-group-item" data-value="<?php echo $comment['id'] ?>">
					<div class="media">
						...
						<section class="media-body">
							...
							<p><small>
								<?php echo (isset ($replied_name_info)? (t('Reply to ') . FSEInfo::getPersonalHomeLink ($replied_name_info, true) . ': ') : '') . h5 ($comment['body']) ?>
							</small></p>
						</section><!-- media-body -->
...
$('#ulComments').on ('click', '.reply-comment', reply_comment);
$('#ulComments').on ('click', '.delete-comment', delete_comment);

$('#formCommentSection').submit (function (e) {
	e.preventDefault();

	$('#btnPostComment').attr ('disabled', 'disabled');
	$('#btnPostComment').bootstrapBtn ('doing');
	...
	var options = {
		success:			function (data) {
			...
			$('#btnPostComment').bootstrapBtn ('reset');
			...
			if (ret_info.status == 'success') {
				...
				$('#btnPostComment').popover ({title: 'Error Occured',
						content: ret_info.detail, triggers: 'manual focus', placement: 'top'});
				$('#btnPostComment').popover ('show');
			}
		},
		error:				function (data) {
