...
class wikiParser
{
	...
	public static function checkParagraph ($wiki_line)
	{
		$start_tag = strtolower (substr ($wiki_line, 0, 3));
		if (in_array ($start_tag, array ('<hr', '<br', '<di',
					'<h1', '<h2', '<h3', '<h4', '<h5', '<h6',
					'<ta', '<tb', '<th', '<tr', '<td', '</t',
					'<ul', '<ol', '<li', '</u', '</o',
					'<dl', '<dt', '<dd',
		...
		foreach($file_parsing_order as $parsing_section_name)
		{
			if(strtoupper($parsing_section_name) == "LINE")
			{
				...
				for($i = 0;$i < count($wiki_lines);$i++)
				{
					...
					{
						$wiki_line = $this->parseSection($parsing_line_section_name,
									$wiki_lines[$i]);
						/* WEIYM: hack to add <p> tag */
						if ($i > 0 && substr ($wiki_line, 0, 2) == '</') {
							$gt_pos = strpos ($wiki_line, '>');
							if ($gt_pos !== false) {
								$close_tag = substr ($wiki_line, 0, $gt_pos + 1);
								$wiki_lines [$i - 1] .= $close_tag;
								$wiki_line = substr ($wiki_line, $gt_pos + 1);
							}
						}
						$wiki_lines[$i] = $this->checkParagraph ($wiki_line);
					}
				}
	...
	private function parseSection($section_name,$wiki_text)
	{
		...
		if(is_array($parser_order_config)
			&& array_key_exists('ParsingDirection', $parser_order_config)
			&& array_key_exists($section_name, $parser_order_config['ParsingDirection'])
			&& strtolower($parser_order_config['ParsingDirection'][$section_name]) == 'reverse')
		{
